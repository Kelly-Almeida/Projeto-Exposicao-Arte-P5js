...
function dia(J){
  ...
  image(bonequinho, width * 0.5, height * 0.7, 200, 200);
  fill(255, 255, 255);
  rect(width * 0.3, 10, width * 0.4, height* 0.1, height * 0.05);
  textSize(32);
  text("Ache o lobinho", width * 0.435, height * 0.07);
  return J;
}
...
function mousePressed() {
  ...
  let clicouNoAstro = mouseX > width * 0.68 && mouseX < width * 0.86 && 
                      mouseY > height * 0.08 && mouseY < height * 0.28;
  ...
  if (clicouNoAstro) {
    ...
    //Bloquei a tela e reinicia o gif
    if (meiaNoite) {
      lobo.setFrame(0);
      lobo.play();
